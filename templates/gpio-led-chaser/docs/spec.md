...
## 4d. 派生文档计数与事实纪律（杜绝 LLM 生成漂移）

本模板的 pipeline 用 LLM 生成 PRD / architecture / development 文档。为通过 `claude-review`
质量门禁，派生文档 **必须** 遵守以下纪律（违反即 blocker）：

1. **禁止自造 FR 编号体系**：PRD 不得建立 `FR-xxx` 编号，不得写 `FRs: N`（P0×a + P1×b …）
   形式的头部计数元数据。功能需求 **直接引用本 spec 的 `Req-001`~`Req-007`**；NFR 单独成节，
   不占用 FR 编号。这样从根上消除“头/正文/附录三处计数互相矛盾”类问题。
2. **禁止机器词频 SHALL 计数**：派生文档不得写 `SHALLs: N`（对全文 SHALL 词频的机器统计），
   该数字与 §4c 的 7 条语义清单是**不同维度**，极易误导下游统计。SHALL 数恒为 7（见 §4c）。
3. **计数自洽硬约束**：若文档确需枚举需求/场景，头部元数据、正文、附录三处的同一计数
   **必须严格一致**；**禁止**在同一文档出现两组互相矛盾的数字；**禁止**声称“已修正”而实际未改
   （诚信/可追溯性 blocker）。
4. **禁止在 codegen 未落地时自报仓库事实**：`codegen-deploy = skipped` 状态下，文档**不得**写
   “源码 N 文件 M 行 / 测试 P 行 / functions X/X / 测试函数 0 / coverage line_rate=?  branch_rate=?”
   等未采集的具体数字（含 `?` 占位）。覆盖率须实测回填或显式标注“待采集”，不得留悬空符号。
5. **测试表述统一口径**：涉及测试须使用 §4b 口径——“`tests/test_main.c` 含 30+ 内联 CHECK 断言、
   `ctest` 全绿”——**禁止**写“测试函数 0”“0 测试用例”“无测试”“functions 10/10”等未核对或
   易误读为“无测试”的措辞。
6. **文档须完整收口、不得截断**：architecture / development 须为完整可交付产物（表格闭合、ADR 有
   结论、任务列表收尾）；development 计划按 `Req-001`~`Req-007` 组织、避免过度拆分任务（控制篇幅，
   避免触达 LLM 输出上限被截断）。
