...
> ## ⚠️ 警示：本矩阵不可作为对外合规证据使用
>
> 本矩阵是 **v1 快照**（生成于 2026-06-29），其中引用的部分测试函数**可能不存在于当前代码库中（待创建）**。
> **当前权威追溯证据以 `yuleosh traceability check` 输出为准**，请勿将本快照用于客户交付或合规审计。
...
> **⚠️ 注意（升级声明）**: 本矩阵是 v1 快照。部分测试函数**可能不存在于当前代码库中（待创建）**，**不可作为对外合规证据使用**。  
> 当前权威追溯证据以 **`yuleosh traceability check` 输出**为准；后续迭代将补全缺失测试并更新本矩阵。
