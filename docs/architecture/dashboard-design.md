...
1. **前端技术**：继续 Next.js 静态导出 vs 引入构建工具（Vite）？
2. **图表库**：自绘 SVG vs Chart.js vs ECharts？
3. **角色粒度**：6 角色是否够？是否需要自定义角色？
4. **日志保留策略**：测试日志存多久？（数据量）
5. **座舱指标**：北极星指标"合规总分"权重如何定？
6. **设备 UI 权限**：Device 只读给 Developer 还是也允许 acquire？
