...
def build_prd_prompt(
    ...
    system_prompt = (
        ...
        "where the spec defines them (e.g. ≤250 ms total pinch response = 200 ms "
        "detection window + 50 ms reversal initiation, 100 mm reversal distance, "
        "1 s cool-down, 300 ms Hall-loss). NEVER leave an AC as an empty "
        "stub or a bare restatement of the requirement.\n"
        "- **数值一致性纪律（2026-08-20 r22 复盘）**: 从 spec 引用时序指标时，SHALL 保留 spec "
        "的分段语义（检测窗口 / 触发时间 / 总响应），不得把总响应误写为任一分段值。"
        "反例（历史根因）：spec SW-004 契约 '200ms 检测窗口 + 50ms 反转 ≈ 250ms 响应'，"
        "AC 若写 '50ms 内检测到防夹' 即把反转触发时间误当作检测时间。"
        "引用指标时优先用 spec 原文数值组合，例如 `≤250ms（200ms 检测 + 50ms 反转启动）`。\n\n"
        "CRITICAL — Out-of-Scope / 计数 / 接口权威纪律 (2026-08-21 r22 p9 复盘):\n"
        "- **Out of Scope 不得声明 spec 未限制的功能**: 只有 spec 明确排除或明确不在范围内的功能才可列入 Out of Scope。"
        "反例（历史根因）：spec SW-004 仅写 'during CLOSING in the pinch zone'（无模式限制，实现 window_control.c:175 "
        "对 MANUAL 与 ONE_TOUCH 均调用 window_modes_check_pinch），PRD 却写 'Anti-pinch in manual mode' 为 Out of "
        "Scope —— 错误裁剪安全功能，claude-review 3 blockers 拦截。"
        "若 spec 未排除某功能，不得自行声明其 Out of Scope；宁可不在该节列出。\n"
...
def build_architecture_prompt(
    ...
    system_prompt = (
        ...
        "IMPORTANT — HAL boundary: the demo templates' HAL functions (hal_can_*/"
        "hal_ble_*/gpio_*/register read-write) are HOST-SIMULATION STUBS, not real "
        "silicon/peripheral drivers. Describe them as a host-simulation seam; do NOT "
        "claim concrete MCU peripherals (real register maps, radio/PHY) are implemented. "
        "Any target-specific deviation (e.g. concrete MCU part number) MUST be listed "
        "in a deviation table, not asserted as done.\n\n"
...
def build_development_prompt(
    ...
    system_prompt = (
        ...
        "- **测试计数口径 (§4b/§4d.4)**: 交付文档中测试套件须 **如实对应 Repository Facts 的"
        "机器收集结果**——框架用真实的 custom-Check 内联断言 harness (非 ctest/CMake CTest,"
        "本仓库 demo 模板未接入), 断言数写真实收集值。不得为迎合 spec"
        "§4b 的 \"≥30\" 谎报阈值或声称 ctest 全绿; 计数漂移/不实框架会被 claude-review 判 blocker。\n"
        "- **HAL 仿真边界 (防幻觉)**: demo 模板的 HAL 函数是宿主机仿真桩, 非真实硅片驱动;"
        "文档须明确标注为 host-simulation seam, 不得声称已落具体 MCU 外设 (真实寄存器映射/"
        "radio/PHY)。目标相关差异 (具体 MCU 型号) 在 deviation 表如实标注。\n"
...
def build_code_review_prompt(
    ...
    system_prompt = (
        ...
        "- BEFORE reporting a config-bound / validation gap, READ the spec's bounds table "
        "(e.g. `cooldownPeriodMs | 1000 | 0 | 10000`): if the spec defines the value's min/max, "
        "a missing check for an impossible value (e.g. uint16 < 0) is NOT a finding — the spec "
        "itself allows that range. Downgrade to info or omit.\n"
