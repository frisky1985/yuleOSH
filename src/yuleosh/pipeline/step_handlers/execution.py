...
def _step_claude_dev_planning(session: PipelineSession) -> str:
    ...
    try:
        ...
        try:
            # max_tokens=14000 (2026-09-14): 实测 DeepSeek(deepseek-flash) 可输出 >8192
            #（探针请求 12000 返回 12000 token），原 8192 上限会把 development-plan.md 在
            # 附录 A 覆盖矩阵中途截断（claude-review §4d.6 blocker）。提至 14000 留足收口余量；
            # 同时 prompts.py 已禁止 LLM 抄写纪律 boilerplate 以压缩篇幅。
            result = _call_llm(session, system_prompt, user_prompt, max_tokens=14000)
        except Exception as e:
