...
def step_review_startup(session: PipelineSession) -> str:
    ...
    try:
        ...
        if is_mock(session):
            print("  ⏭️  [小克] 启动代码审查跳过 — mock 模式")
            return write_mock_skip(
                session, "review-startup",
        ...
        _deploy_skip = maybe_skip_code_review(session, 'review-startup', reviewer="小克")
        if _deploy_skip:
            print(f"  ⏭️  [小克] 启动代码审查跳过 — 本次 run 无代码部署")
            return _deploy_skip
