...
"""LDRA Testbed 扫描器适配器（商业工具，支持 C:2023）。
...
配置（.yuleosh/ci-config.yaml）::
    ...
    misra:
      scanner: ldra
      scanner_config:
        cli_path: "tbx"               # LDRA CLI 可执行名或绝对路径
...
# LDRA severity → 统一 severity（对齐 cppcheck 档位）
_SEVERITY_MAP = {
    "mandatory": "error",
    "required": "required",
    "advisory": "advisory",
...
# 行格式：RULE|DIR X.Y, file, line N, Severity: message
_PATTERN_LINE = re.compile(
    r"^(?P<kind>RULE|DIR)\s+(?P<num>[\d.]+)\s*,\s*(?P<file>.+?)\s*,\s*"
    r"line\s+(?P<line>\d+)\s*,\s*(?P<severity>[^:]+):\s*(?P<message>.*)$"
)


class LdraScannerAdapter(ScannerAdapter):
    """LDRA Testbed 适配器（CLI/文本解析）。"""

    name = "ldra"
    display_name = "LDRA Testbed"
    ...
    def detect_hint(self, project_dir: str, config: Any = None) -> str:
        return ("install LDRA Testbed or set misra.scanner_config.report_file "
                "to an existing LDRA text report")
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        if report_file:
            ...
            if not os.path.isfile(rp):
                return ScannerResult(
                    ...
                    error=f"ldra report file not found: {rp}",
                    hint="set misra.scanner_config.report_file to an existing LDRA text report",
                )
            try:
        ...
        try:
            ...
            return ScannerResult(
                tool=self.name, ok=False,
                error=f"ldra CLI not found: {cli_path}",
                hint="install LDRA Testbed or set misra.scanner_config.cli_path",
            )
        except subprocess.TimeoutExpired:
            return ScannerResult(
                tool=self.name, ok=False,
                error=f"ldra CLI timed out after {timeout}s",
                hint="increase misra.scanner_config.timeout or reduce file count",
            )
        except Exception as e:  # noqa: BLE001 — 子进程边界防御（外部工具异常兜底）
            return ScannerResult(
                tool=self.name, ok=False,
                error="ldra execution error: " + str(e),
                hint="check LDRA installation and configuration",
            )
    ...
    def parse(self, raw: str) -> list[Violation]:
        ...
        for line in raw.splitlines():
            ...
            num = m.group("num")
            rule = f"{'Dir' if kind == 'DIR' else 'Rule'} {num}"
            try:
