 ...
 * Use this as a placeholder until the full WdgIf driver is integrated.
 */
...
extern Std_ReturnType WdgIf_SetTriggerCondition(uint8_t DeviceIndex, uint16_t TimeoutMs);

extern Std_ReturnType WdgIf_SetMode(uint8_t DeviceIndex, WdgIf_ModeType Mode);

extern WdgIf_ModeType WdgIf_GetMode(uint8_t DeviceIndex);
