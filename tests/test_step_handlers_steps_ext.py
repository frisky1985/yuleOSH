...
class TestStepCUnitTest:
    def test_no_c_files_skips(self, mock_session, tmp_path):
        ...
        # fixture 已设 session.project_dir = tmp_path (无 .c 文件)
    ...
    def test_has_c_files_no_tests(self, mock_session, tmp_path):
        """When .c files exist but no test framework, status is 'skipped'."""
...
class TestStepIntegrationTest:
    ...
    def test_pytest_runner_success(self, mock_session, spec_file, tmp_path):
        mock_session.spec_path = str(spec_file)
        # 有 tests/ 目录才能进入 pytest 分支 (fixture 的 tmp_path 默认没有)
    ...
    def test_pytest_runner_failure(self, mock_session, spec_file, tmp_path):
        mock_session.spec_path = str(spec_file)
        proj_dir = tmp_path / "pyproj2"
        proj_dir.mkdir()
        (proj_dir / "tests").mkdir()
        mock_session.project_dir = proj_dir
    ...
    def test_pytest_timeout(self, mock_session, spec_file, tmp_path):
        """pytest timeout should be handled."""
        mock_session.spec_path = str(spec_file)
        proj_dir = tmp_path / "pyproj3"
        proj_dir.mkdir()
        (proj_dir / "tests").mkdir()
        mock_session.project_dir = proj_dir
